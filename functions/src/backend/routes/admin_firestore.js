...
router.get('/logs', async (req, res) => {
  try {
    ...
    // Avoid composite index requirements: query by orderBy only, then filter in memory
    ...
    try {
      ...
      // Fallback: if index error, try without orderBy
      console.warn('Audit logs orderBy failed, using fallback:', indexErr.message);
      snap = await db
        .collection('audit_logs')
        .limit(fetchLimit)
        .get();
